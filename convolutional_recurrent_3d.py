...
class ConvRNN3D(RNN):
    ...
    # Input shape
        ...
        `(samples, timesteps, channels, rows, cols, hgts)` if data_format='channels_first'
        or 5D tensor with shape:
        `(samples, timesteps, rows, cols, hgts, channels)` if data_format='channels_last'.
    # Output shape
        - if `return_state`: a list of tensors. The first tensor is
            the output. The remaining tensors are the last states,
            each 6D tensor with shape:
            `(samples, timesteps,
              filters, new_rows, new_cols, new_hgts)` if data_format='channels_first'
            or 6D tensor with shape:
            `(samples, timesteps,
              new_rows, new_cols, new_hgts, filters)` if data_format='channels_last'.
        ...
        - else, 5D tensor with shape:
            `(samples, filters, new_rows, new_cols, new_hgts)` if data_format='channels_first'
            or 5D tensor with shape:
            `(samples, new_rows, new_cols, new_hgts, filters)` if data_format='channels_last'.
    # Masking
            ...
            - specify a fixed batch size for your model, by passing
                 ...
                 - if functional model with 1 or more Input layers:
                    ...
                    *including the batch size*.
                    It should be a tuple of integers, e.g. `(32, 10, 100, 100, 100, 32)`.
                    Note that the number of rows, columns and heights should be specified too.
    ...
    def compute_output_shape(self, input_shape):
        ...
        hgts = conv_utils.conv_output_length(hgts,
                                             cell.kernel_size[2],
                                             padding=cell.padding,
                                             stride=cell.strides[2],
                                             dilation=cell.dilation_rate[2])

        output_shape = input_shape[:2] + (rows, cols, hgts, cell.filters)
        ...
        if self.return_state:
            ...
            base = (input_shape[0], rows, cols, hgts, cell.filters)
...
class ConvLSTM3D(ConvRNN3D):
    ...
    #@interfaces.legacy_convlstm3d_support
    def __init__(self, filters,
                 kernel_size,
                 strides=(1, 1, 1),
                 padding='valid',
